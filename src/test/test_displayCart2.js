...
function displayCart() {
  ...
  if (cartData.length === 0) {
    displayEmptyCartMessage(cartItemsContainer);
  } else {
    renderCartItems(cartItemsContainer, cartData);
  }
}
...
const displayEmptyCartMessage = (container) => {
  container.innerHTML = "<p>Your cart is empty.</p>";
};

const renderCartItems = (container, items) => {
  items.forEach((item) => {
...
// Test suite for displayCart function
